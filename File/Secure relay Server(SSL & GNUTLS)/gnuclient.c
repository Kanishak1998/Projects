...
int main(int argc, char **argv)
{
    ...
    *connfdPtr = connfd;
	...
	scanf("%d",&v);
	bzero(buffer,256);
	int n = gnutls_record_recv(session,buffer,255);
	if (n < 0) 
	...
	switch(v)
	{
		case 1:
		{
			...
			bzero(buffer,1025);
			scanf("%s",buffer);
			n=gnutls_record_send(session,buffer, strlen(buffer));
			if (n < 0) 
             			perror("ERROR writing to socket");
			bzero(buffer,1025);
			n = gnutls_record_recv(session,buffer,1024);
			if (n < 0) 
			     perror("ERROR reading from socket");
			printf("Server : %s\n",buffer);
			bzero(buffer,1025);
			...
			gnutls_record_send(session, &words, sizeof(int));
			rewind(f);
			...
			while((ch = fgetc(f)) != EOF)
	      		{
				gnutls_record_send(session,&ch,sizeof(char));
				printf("%c",ch);
		...
		case 2:
		{
			...
			fp = fopen("recv.txt","a");            
			int words;
			gnutls_record_recv(session, &words, sizeof(int));
			printf("Passed integer is : %d\n" , words);      //Ignore , Line for Testing
			while(ch != words)
			{
				char c;
				gnutls_record_recv(session,&c,sizeof(char));
