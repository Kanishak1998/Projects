...
{	
			...
			while(1)
			{
				...
				rsd=client_socket[w];
				n = write(rsd,buffer,strlen(buffer));
				...
				char id;
				id=buffer;
				printf("message delivered to");
				int fd_id=-1;*//*
				for(int j=0;j<max_clients; j++)
				{
					if(strcmp(name[j][1025],id[1025])==0)
					{
						fd_id=j;	
					}
				}
				...
				int rsd=client_socket[0];
				int n = write(sd,"Enter Message",strlen("Enter Message"));
			   	if (n < 0) perror("ERROR writing to socket");
				bzero(buffer,256);
