		...
		{ 
			...
			{ 
				...
				{ 
					...
					char q;
					//tak the initial letter as the id of the user	
					q=substring(buffer);
					name[i]=q;
...
{	
				...
				int w=fd_id;//store reciver id in w
			...
			while(1)
			{
				...
				bzero(buffer,256);
				valread = gnutls_record_recv(session[i] , buffer, 255);//rec msg from the init client
				rsd=client_socket[w];
				n = gnutls_record_send(session[w],buffer,strlen(buffer));//forward the msg to the reciver client
				fputs("Message delivered\n", stdout);				
				if (n < 0) perror("ERROR writing to socket");
				//write(sd,"Message Delivered",strlen("Message Delivered"));
				bzero(buffer,256);
				gnutls_record_recv(session[w] ,buffer, 255);//take reply from the reciver
				fputs("Message reciverd\n", stdout);
				sd = client_socket[i]; 
				n=gnutls_record_send(session[i],buffer,strlen(buffer));//forward it to the sendeer
