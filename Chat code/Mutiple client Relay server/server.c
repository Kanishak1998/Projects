			...
			{ 	
				...
				{	
					...
					while(1)
					{
						...
						bzero(buffer,256);
						valread = read(sd, buffer, 255);//rec msg from the init client
						rsd=client_socket[w];
						n = write(rsd,buffer,strlen(buffer));//forward the msg to the reciver client
						fputs("Message delivered\n", stdout);				
						if (n < 0) perror("ERROR writing to socket");
						//write(sd,"Message Delivered",strlen("Message Delivered"));
						bzero(buffer,256);
						read(rsd,buffer, 255);//take reply from the reciver
						fputs("Message reciverd\n", stdout);
						sd = client_socket[i]; 
						n=write(sd,buffer,strlen(buffer));//forward it to the sender
